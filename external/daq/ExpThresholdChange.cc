...
#include "external/pixie/app/xia_common.h"
...
void ExpThresholdChange::Scan_Pars()
{
  ...
  if(parsthreBox->GetSelected()==0)
	...
	expthreread.open("parset/exp/DSPpar.thre");
      if(parsthreBox->GetSelected()==2)
	expthreread.open("parset/exp/user0.thre");
      if(parsthreBox->GetSelected()==3)
	expthreread.open("parset/exp/user1.thre");
      if(parsthreBox->GetSelected()==4)
	expthreread.open("parset/exp/user2.thre");
      if(parsthreBox->GetSelected()==5)
	expthreread.open("parset/exp/user3.thre");
      if(parsthreBox->GetSelected()==6)
	expthreread.open("parset/exp/user4.thre");
	    
      if(!expthreread.is_open())
	{
	  std::cout<<"can't open file -- parset/exp/*.thre"<<std::endl;
	  return;
	}
...
void ExpThresholdChange::Save_Pars()
{
  ...
  if(parsthreBox->GetSelected()==2)
    expthrewrite.open("parset/exp/user0.thre");
  if(parsthreBox->GetSelected()==3)
    expthrewrite.open("parset/exp/user1.thre");
  if(parsthreBox->GetSelected()==4)
    expthrewrite.open("parset/exp/user2.thre");
  if(parsthreBox->GetSelected()==5)
    expthrewrite.open("parset/exp/user3.thre");
  if(parsthreBox->GetSelected()==6)
    expthrewrite.open("parset/exp/user4.thre");
