...
>  [!WARNING]
>
> 必须注意这部分的内容，获取程序的组编号、共享内存大小和符合窗的宽度是**必须**根据实验情况调整的内容，会直接影响在线程序的正常运行。
...
获取程序和在线程序时协同工作的，**组策略**是两边程序的共同机制，所以在线程序设置了组编号的同时，也要在获取程序这边设置组编号。两边的组编号应该设置为一模一样。

获取程序的组编号在 `external/daq/Detector.cc` 中的 `Detector::Detector` 中设置，变量名为 `group_index_`
